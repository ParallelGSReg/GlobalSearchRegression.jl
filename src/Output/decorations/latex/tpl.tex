...
\begin{lstlisting} 
      ...
      GlobalSearchRegression.gsr(
        ...
        {{#panel}}panel = {{.}},{{/panel}}
        {{#time}}time = {{.}},{{/time}}
        {{#seasonaladjustment}}seasonaladjustment = {{.}},{{/seasonaladjustment}}
...
\begin{enumerate}
  ...
  {{#seasonalfortex}}
    \item Seasonal-adjustment: variables {{#:fe_seasonaladjustment_names}}{{.}}, {{/:fe_seasonaladjustment_names}}have been selected for seasonal adjustment using \verb SingularSpectrumAnalysis.jl  (see \cite{hassani2007}). For each one, the function removes the seasonal component using window lengths equal to {{#:fe_seasonaladjustment_vals}}{{.}}, {{/:fe_seasonaladjustment_vals}}respectively.
  {{/seasonalfortex}}

  {{#seasonalfortex2}}
  \item Seasonal-adjustment: variable {{#:fe_seasonaladjustment_names2}}{{.}}{{/:fe_seasonaladjustment_names2}} has been selected for seasonal adjustment using \verb SingularSpectrumAnalysis.jl  (see \cite{hassani2007}). The function removes the seasonal component using a window length equal to {{#:fe_seasonaladjustment_vals2}}{{.}}{{/:fe_seasonaladjustment_vals2}}.
  {{/seasonalfortex2}}
...
\begin{table}[!h]
  ...
  \caption{GSReg results}
    ...
    \hline{{#bmexpvars}} 
    {{name}}     & {{best.b}}{{best.stars}} {{#avg}} & {{b}}{{stars}}    {{/avg}} \\{{#ttest}}
                 & {{best.bstd}}            {{#avg}} & {{bstd}} {{/avg}} \\{{/ttest}}
    {{/bmexpvars}}\hline

    Observations &   \multicolumn{ {{^avg}}1{{/avg}} {{#avg}}2{{/avg}} }{c}{ } \\
    Criteria     &   \multicolumn{ {{^avg}}1{{/avg}} {{#avg}}2{{/avg}} }{c}{ } \\
    \hline
    \hline
